...
class TaskParams:
    ...
    robot_initial_joint_pos = np.array([1.3, -2.0, 2.0, -1.5, -1.5, 0.0, 0.0, 0.0]) # With gripper joint pos set to 0.0
    ...
    gripper_offset = torch.tensor([0, 0, 0.15], device="cuda") # or torch.tensor([0, 0, 0.135], device="cuda")
    gripper_open = torch.tensor([0.0, 0.0])
    gripper_close = torch.tensor([-0.025, -0.025])
    ...
    hole_randomize_pose_range = {"x": (-0.01, 0.01), "y": (-0.01, 0.01), "z": (0.0, 0.0)}, # , "yaw": (-math.pi, math.pi) #